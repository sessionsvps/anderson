...
async def logUserBot():
    load_dotenv()
    api_id = int(29589147)
    api_hash = "3151ae3e52d4ee15322fea3f068a8f1f"
    phone_number = "51933797226"
    ...
    await client.send_message("@spmanderson", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4533432624")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmanderson", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmanderson", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmanderson", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(90)
                        if j==2: break
            await client.send_message("@spmanderson", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
